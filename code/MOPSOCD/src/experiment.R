...
run <- function(problem = 5){
	...
	fitFunction <- function(chromosome, gen) {
		if(gen != 50){
			#threshold <- threshold - (0.5 / (50 - gen))
			threshold <- runif(1, 0.5, 0.7)
		}
	...
	foreach(iter = 1:40) %dopar%{
		filename_mpso_time <- paste('../result/', problem, '/', iter, "_mpso_time.csv", sep = '')
		filename_mpso <- paste('../result/', problem, '/', iter, "_mpso.csv", sep = "")
		mPSOtime <- vector()
		cat("Iteration: ", iter, '\n')
		ptm <- proc.time()
		#mPSO begins here
		mpsoResult <- mopsocd(fitFunction, varcnt=varcount,
					  ...
					  opt=optim, maxgen=maxgen, popsize=popsize, seed = iter, problem = problem)
	...
	mPSOtimeFilename <- paste('../result/', problem, '/', problem, "_mPSO_time.csv", sep='')

	write.csv(best_mpso, filename, row.names = F, quote = F)
