...
mopsocd <- function (fn, gn = NULL, varcnt, fncnt, lowerbound, upperbound, 
    ...
    pMut = 0.5, w = 0.4, c1 = 1, c2 = 1, seed, problem, logS = F) 
{
    ...
    initialize_pop <- function() {
        for (i in 1:varcnt) {
            popVar[1:popsize, i] <<- runif(popsize, min = minvalue[i], 
                max = maxvalue[i])
        }
    }
    ...
    evaluate <- function(gen) {
        popFit[1:popsize, ] <<- t(apply(popVar, 1, fn, gen))
    }
    ...
    crowding <- function() {
        ...
        for (i in seq(fncnt)) {
            sorted_archive <- sort(archiveFit[1:ndomCtr, i], 
                index.return = TRUE)
            valsa <- sorted_archive$x
            ixsa <- sorted_archive$ix
            crowdDist[ixsa[2:(ndomCtr - 1)]] <<- crowdDist[ixsa[2:(ndomCtr - 
                1)]] + valsa[3:ndomCtr] - valsa[1:(ndomCtr - 
                2)]
            maxcd <- which.max(crowdDist[1:ndomCtr])
            crowdDist[ixsa[1]] <<- crowdDist[ixsa[1]] + crowdDist[maxcd]
            crowdDist[ixsa[ndomCtr]] <<- crowdDist[ixsa[ndomCtr]] + 
                crowdDist[maxcd]
            mincd <- which.min(crowdDist[1:ndomCtr])
            crowdDist[1:ndomCtr] <<- crowdDist[1:ndomCtr]/(crowdDist[maxcd] - 
                crowdDist[mincd])
        }
        sorted_cd <- sort(crowdDist[1:ndomCtr], decreasing = TRUE, 
            index.return = TRUE)
