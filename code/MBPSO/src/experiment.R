...
run <- function(problem = 5){
	...
	foreach(iter = 1:40) %dopar%{
		cat("Generation: ", iter,"\n")
		filename_mbpso_time <- paste('../result/', problem, '/', iter, '_mbpso_time.csv', sep = '')
		filename_mbpso <- paste('../result/', problem, '/', iter, '_mbpso.csv', sep = '')
		ptm <- proc.time()
		mbPSOdata <- mbpso(fitFunc, varcount, obj, lbound, ubound, c1, c2, maxgen, popSize, iter, problem)
		mbPSOtime <- c(mbPSOtime, (proc.time() - ptm)[1])
		write.csv(mbPSOdata, filename_mbpso, row.names = F, quote = F)
		write.csv(mbPSOtime, filename_mbpso_time, row.names = F, quote = F)
	}
	print("Compiling...")
	for(iter in 1:40){
		filename_mbpso <- paste('../result/', problem, '/', iter, "_mbpso.csv", sep = "")
		filename_mbpso_time <- paste('../result/', problem, '/', iter, '_mbpso_time.csv', sep = "")
		mbPSOdata <- rbind(mbPSOdata, read.csv(filename_mbpso, header = T, sep = ','))
		mbPSOtime <- rbind(mbPSOtime, read.csv(filename_mbpso_time, header = T))
	}
	#best front
	best_mbpso <- best_front(mbPSOdata)
	names(best_mbpso) <- c('costF', 'latencyF', 'rank')
	...
	#write final result
	write.csv(best_mbpso, filename, row.names = F, quote = F)
	write.csv(mbPSOdata, timeFilename, row.names = F, quote = F)
	#---------------Run Algorithm End---------------------
}
