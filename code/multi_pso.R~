algorithm <- function (fn, gn = NULL, varcnt, fncnt, lowerbound, upperbound, 
		  ...
		  pMut = 0.5, w = 0.4, c1 = 1, c2 = 1) 
{
	...
	#customazed initialize_pop-----------------------------------------------------------------
	#initialize_pop <- function() {
		#for (i in popsize) {
			#popVar[i, ] <<- rbinom(varcnt, 1, 0.5)
		#}
	#}
	#------------------------------------------------------------------------------------------
	...
	insert_nondom <- function() {
		if (!is.null(gn)) {
			...
			for (i in ix) {
				if (opt == 1) {
					x <- (t(matrix(rep(popFit[i, ], lenix), nrow = fncnt)) - popFit[ix, ] < 0) * 1
				}
				else {
					x <- (t(matrix(rep(popFit[i, ], lenix), nrow = fncnt)) - popFit[ix, ] > 0) * 1
				}
	...
	update_archive <- function() {
		...
		for (i in ixfpop) {
			if (opt == 1) {
				x <- (t(matrix(rep(popFit[i, ], ndomCtr), nrow = fncnt)) - archiveFit[1:ndomCtr, ] < 0) * 1
			}
			else {
				x <- (t(matrix(rep(popFit[i, ], ndomCtr), nrow = fncnt)) - archiveFit[1:ndomCtr, ] > 0) * 1
			}
		...
		for (i in ixarc) {
			...
			if (opt == 1) {
				x <- (t(matrix(rep(archiveFit[i, ], ndomCtr), 
							   nrow = fncnt)) - archiveFit[ixarc, ] < 0) * 1
			}
			else {
				x <- (t(matrix(rep(archiveFit[i, ], ndomCtr), 
							   nrow = fncnt)) - archiveFit[ixarc, ] > 0) * 1
	...
	compute_velocity <- function() {
		...
		c1 * randum1 * (pbestsVar - popVar) + c2 * randum2 * (gBestmat - popVar)
	...
	while (t < maxgen) {
		t <- t + 1
		if (verbosity >= 1) cat("Generation ", t, "\n")
		if (ndomCtr > 2 * fncnt) crowding()
		compute_velocity()
		maintain_particles()
		mutate()
		evaluate()
