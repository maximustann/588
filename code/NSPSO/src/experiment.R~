...
run <- function(problem = 5, logS = F){
	...
	#fire on all 8 cores
	print("Start")
	dir.create(paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/run/', sep = ''), showWarnings = F)
	dir.create(paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/best/', sep = ''), showWarnings = F)
	foreach(iter = 1:40) %dopar%{
		cat("run: ", iter,"\n")
		filename_nspso_time <- paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/run/', iter, '_nspso_time.csv', sep = '')
		filename_nspso <- paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/run/', iter, '_nspso.csv', sep = '')
		ptm <- proc.time()
		nsPSOdata <- nspso(fitFunc, varcount, obj, lbound, ubound, c1, c2, maxgen, popSize, iter, problem, logS)
		nsPSOtime <- c(nsPSOtime, (proc.time() - ptm)[1])
		write.csv(nsPSOdata, filename_nspso, row.names = F, quote = F)
		write.csv(nsPSOtime, filename_nspso_time, row.names = F, quote = F)
	}
	...
	for(iter in 1:40){
		filename_nspso <- paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/run/', iter, "_nspso.csv", sep = "")
		filename_nspso_time <- paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/run/', iter, '_nspso_time.csv', sep = "")
		nsPSOdata <- rbind(nsPSOdata, read.csv(filename_nspso, header = T, sep = ','))
		nsPSOtime <- rbind(nsPSOtime, read.csv(filename_nspso_time, header = T))
	}
	totalFrontNum <- nrow(nsPSOdata)
	eachCoreNum <- floor(totalFrontNum/8) # 8 cores
	#best front
	tempBestfront <- foreach(iter = 1:8, .combine=rbind) %dopar%{
						if(iter == 8){
							best_front(nsPSOdata[(1 + (iter - 1) * eachCoreNum):nrow(nsPSOdata), ])
						} else{
							best_front(nsPSOdata[(1 + (iter - 1) * eachCoreNum):(iter*eachCoreNum), ])
						}
					}
	#best front
	best_nspso <- best_front(tempBestfront)
	names(best_nspso) <- c('costF', 'latencyF', 'rank')

	#prepare file name 
	filename <- paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/best/', problem, '.csv', sep = '')
	timeFilename <- paste('/home/st-james1/tanboxi/588_project/code/NSPSO/result/', problem, '/best/', problem,'_time.csv', sep = '')

	#write final result
	write.csv(best_nspso[, 1:2], filename, row.names = F, quote = F)
	write.csv(nsPSOdata, timeFilename, row.names = F, quote = F)
	#---------------Run Algorithm End---------------------
}
